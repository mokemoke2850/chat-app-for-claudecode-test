...
複数 Issue を同時に進める場合は、ファイル競合、DB スキーマ競合、AI レビュー漏れを防ぐためにフェーズを分ける。
...
- Issue ごとに作成 worker と独立 AI reviewer を分ける
- テスト設計は `APPROVED` まで自動反復し、人間確認や確認専用 Draft PR を挟まない
- 画面影響がある Issue は実装検証と、該当する場合の高リスクレビュー後に Playwright 確認を行う
...
1. worker がテスト観点と `it.todo` を作る
2. 独立 reviewer が `doc/ai-review-tdd-guide.md` に従ってレビューする
3. `CHANGES_REQUESTED` は同じ worker と reviewer で修正・再レビューする
4. `APPROVED` 後にテストとプログラムを実装する
5. 高リスク変更は実装レビュー、画面影響がある変更は Playwright 確認を行う
6. 検証結果を記録した通常 PR を作る

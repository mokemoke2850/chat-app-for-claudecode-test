...
- [ ] **キーボードショートカット一覧モーダル** — `Cmd+/` または `?` で表示。学習コストを下げるオンボーディング兼用｜難易度: 低
- [ ] **j/k 等のメッセージリストキーバインド** — マウスを使わずメッセージ間を移動・スレッド展開・リアクション付与できる｜難易度: 中
- [ ] **チャンネル切替時のスクロール位置記憶** — チャンネルを行き来しても直前の位置に戻る。読み返しの体験を向上｜難易度: 中
- [ ] **サイドバー幅のドラッグリサイズと永続化** — ChannelList 側ペインを利用者好みに調整できる｜難易度: 低
- [ ] **Rail の折り畳み状態を永続化** — `localStorage` に保持して再訪時のレイアウトを保つ（ContextRail は実装済）｜難易度: 低
- [ ] **受信箱サマリーカードのドリルダウン** — 「未読 0」「今日の予定 0」「未完タスク 0」をクリックで該当ビューへ遷移させる｜難易度: 低

### メッセージ作成・編集体験

- [ ] **エディタへの画像クリップボード貼り付け** — スクリーンショットを `Cmd+V` で添付可能に｜難易度: 中
- [ ] **エディタへのドラッグ&ドロップ添付** — ファイルを直接ドロップしてアップロード｜難易度: 低
- [ ] **送信前 Markdown プレビュー切替** — 表示崩れを送信前に確認できる｜難易度: 低
- [ ] **メッセージのリンクコピー** — 個別メッセージへのパーマリンクをクリップボードへ。Slack 互換 UX｜難易度: 低
- [ ] **未送信添付の自動下書き連携** — 下書き保存時に添付メタデータも合わせて復元する（既存 #148 の拡張）｜難易度: 中
- [ ] **絵文字ピッカーの拡充** — スキントーン・最近使った・検索タブ・カテゴリの整理｜難易度: 中
- [ ] **メンションサジェストに `@here` `@channel` を追加** — オンライン中／全員宛の擬似メンションを通知ルールと連動させる｜難易度: 中

### 通知・受信箱

- [ ] **アプリ内通知センター** — Bell アイコンから過去の通知（メンション・リマインダー・予約送信失敗・モデレーション結果）を時系列で確認できる｜難易度: 中
- [ ] **未読／既読フィルタトグル（受信箱）** — タブ内で未読のみ／全件を切り替えられる｜難易度: 低
...
- [ ] **タスクとカレンダーの双方向連携** — 期限付きタスクをカレンダー上にも表示し、ドラッグで期限変更できる｜難易度: 中
- [ ] **タスクボードのインライン作成** — 各カラムから直接タスクを追加できる（現状はモーダル必須）｜難易度: 低
- [ ] **タスクのサブタスク／依存関係** — 親子関係や前後関係を持たせて簡易ガントを描画する｜難易度: 中

### ブックマーク・テンプレート・検索

- [ ] **ブックマーク内検索とタグ付け** — 件数が増えても発見性を保つ。タグやフォルダで分類できる｜難易度: 中
- [ ] **ブックマークから元メッセージへのジャンプ** — クリックで該当チャンネルの当該メッセージへ遷移しハイライト表示する｜難易度: 低
- [ ] **テンプレートのカテゴリ／共有スコープ** — 個人 / チャンネル / ワークスペースのスコープと、フォルダ階層を持たせる｜難易度: 中
- [ ] **テンプレートに変数プレースホルダ** — `{{user}}` `{{date}}` などを差し込んで定型文を動的生成する｜難易度: 中
- [ ] **検索結果のグルーピング表示** — チャンネル別・送信者別にまとめて視認性を上げる｜難易度: 中

### 管理 / 運用

- [ ] **管理ダッシュボードのグラフ化** — 投稿数・アクティブユーザー・チャンネル別ボリュームの時系列グラフを追加する｜難易度: 中
- [ ] **管理ダッシュボードの期間切替** — 24h / 7d / 30d / カスタムを切替できる｜難易度: 低
- [ ] **ワークスペース全体の使用状況レポート CSV** — 月次の利用状況を CSV で書き出す｜難易度: 低
...
- [ ] **アクセントカラー / カスタムテーマ** — ユーザーごとにライト・ダークの色相を選択できる｜難易度: 中
- [ ] **コントラスト & フォントサイズ調整** — WCAG AA 準拠のハイコントラストモードと文字サイズ切替｜難易度: 低
- [ ] **メッセージ密度切替（コンパクト／快適）** — Slack 互換のレイアウト密度オプション｜難易度: 低

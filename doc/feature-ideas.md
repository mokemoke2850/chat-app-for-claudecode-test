...
- [ ] **Rail アイコンの常時ラベル表示モード** — 現状はアイコンのみで、初見では受信箱・チャット・DM・タスク等の意味をホバーしないと判断しづらい。折り畳み Rail とは別に「アイコン + 短いラベル」の幅広モードを用意する｜難易度: 低
- [ ] **チャンネル未選択時の次アクション CTA** — `/chat` 初期表示の「チャンネルを選択してください」に、最近使ったチャンネル・未読チャンネル・チャンネル作成への導線を追加する｜難易度: 低
- [ ] **ページ別サイドバー表示ポリシーの整理** — 受信箱・検索・タスク・カレンダーでも ChannelList が常時表示されるため、主作業と無関係な領域が目立つ。ページごとに「関連サイドバー」「折り畳み既定」「非表示」を明確化する｜難易度: 中

### 受信箱・通知

- [ ] **受信箱の空状態をアクション付きにする** — 「未読のメンションはありません」だけで終わらせず、未読のみ解除・すべてタブ表示・通知設定確認への小さなボタンを出す｜難易度: 低
- [ ] **サマリーカードの意味を補足するミニ内訳** — 「未読 1」なのにメンション一覧は空、のような状態で理由が分かりにくい。未読の内訳（チャンネル、DM、スレッド等）をカード内またはホバーで表示する｜難易度: 中
- [ ] **通知許可 CTA の状態別ガイダンス** — Rail 下部の通知アイコンは有効化の必要性が伝わりづらい。ブラウザ権限未設定・拒否済み・有効の状態ごとに文言と導線を切り替える｜難易度: 低

### チャット作成体験

- [ ] **メッセージ入力欄の送信ボタンを明示する** — エディタ右下の紙飛行機アイコンだけでは、予約送信・添付・絵文字との優先関係が分かりづらい。入力後は主要アクションとして「送信」ラベル付きボタンを強調する｜難易度: 低
- [ ] **エディタツールバーのグループ見出し / ツールチップ強化** — Bold/Italic 等のアイコンは揃っているが、画像・コード・整形解除などが初見で判別しづらい。ツールチップの説明とキーボードショートカット表示を統一する｜難易度: 低
- [ ] **イベントカードの RSVP 結果を視覚的に読みやすくする** — チャット内イベントの「参加 / 不参加 / 未定」が小さいボタン列で、現在の自分の選択状態や合計が埋もれる。選択済み状態、合計、参加者プレビューをカード内で強調する｜難易度: 中

### 検索

- [ ] **検索ページのヘッダーと保存ビュー領域を明示する** — 画面上部が検索入力から始まり、ページタイトルや保存ビューの存在が見えにくい。`検索` 見出し、保存ビュー行、検索構文ヘルプを整理して情報階層を作る｜難易度: 低
- [ ] **検索構文の入力補完** — `from:` `in:` `has:` `tag:` を手入力する前提が強い。入力中に候補チップ、チャンネル名、ユーザー名、タグ候補をインライン補完する｜難易度: 中
- [ ] **検索結果なしの改善提案** — 検索前のヘルプはあるが、検索後に結果がない場合の次手が弱い。条件を広げる、日付を外す、添付フィルタを解除する等のワンクリック提案を出す｜難易度: 低

### タスク・カレンダー

- [ ] **タスクボードのカラム別 WIP / 期限サマリー** — カラム見出しは件数のみで、期限切れや今日期限の把握に視線移動が必要。各カラムに「期限切れ」「今日」「担当自分」の小バッジを追加する｜難易度: 中
- [ ] **タスクカードのドラッグハンドル分離** — カード全体がドラッグ対象のため、編集・削除・カレンダー表示アイコンとの操作意図が競合しやすい。左端に専用ハンドルを置き、カード本文クリックは詳細表示に割り当てる｜難易度: 中
- [ ] **カレンダー月表示のイベント密度改善** — 月表示ではイベント名が横長バーで並び、チャンネルやタスク由来の違いが色だけに寄りやすい。イベント種別アイコン、チャンネル略称、今日の予定との連動ハイライトを追加する｜難易度: 中
- [ ] **カレンダーのチャンネルフィルタ一括操作** — チャンネル数が増えると個別チェックが重い。全選択 / 全解除 / 未読関連のみ / 自分の参加予定のみ、のプリセットを追加する｜難易度: 低

...
- `users` テーブルに 3 列追加（`status_emoji` / `status_text` / `status_expires_at`）。履歴テーブルは作らない
- 絵文字ピッカーは既存 `EmojiPicker.tsx` を再利用
- 「今日中」「明日まで」などの相対期限は **クライアント側のローカル TZ で計算して UTC で送信**。サーバはタイムゾーン推測しない
- メンション通知の特殊扱いは MVP では実装しない
- 空ステータスの扱い: 絵文字のみ / テキストのみも許容。両方空ならクリア扱い

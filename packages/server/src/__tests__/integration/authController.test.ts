...
/**
 * オンボーディング関連の HTTP テスト（Issue #114）
 */
describe('POST /api/auth/onboarding/complete', () => {
  it('正常: 認証済ユーザーが呼ぶと 200 を返し onboardingCompletedAt が設定される', async () => {
    const { userId } = await registerAndGetCookie(app, 'onb_user1', 'onb1@example.com');
    const token = generateToken(userId, 'onb_user1');
  ...
  it('正常: 2 回呼んでも 200 を返す（冪等性）', async () => {
    const { userId } = await registerAndGetCookie(app, 'onb_user2', 'onb2@example.com');
    const token = generateToken(userId, 'onb_user2');
  ...
  it('正常: 呼び出し後の me レスポンスで onboardingCompletedAt が ISO 文字列になる', async () => {
    const { userId } = await registerAndGetCookie(app, 'onb_user3', 'onb3@example.com');
    const token = generateToken(userId, 'onb_user3');

    await request(app).post('/api/auth/onboarding/complete').set('Cookie', `token=${token}`);
    const me = await request(app).get('/api/auth/me').set('Cookie', `token=${token}`);

    expect(me.status).toBe(200);
    expect(me.body.user.onboardingCompletedAt).not.toBeNull();
    expect(Number.isNaN(new Date(me.body.user.onboardingCompletedAt).getTime())).toBe(false);
  });

  it('異常: 非ログイン状態は 401 を返す', async () => {
    const res = await request(app).post('/api/auth/onboarding/complete');
    expect(res.status).toBe(401);
  });
});

describe('GET /api/auth/me（オンボーディング関連の追加項目）', () => {
  it('未完了ユーザーのレスポンスに onboardingCompletedAt: null が含まれる', async () => {
    const { userId } = await registerAndGetCookie(app, 'onb_me1', 'onb_me1@example.com');
    const token = generateToken(userId, 'onb_me1');
  ...
  it('完了済ユーザーのレスポンスに onboardingCompletedAt が ISO 文字列で含まれる', async () => {
    const { userId } = await registerAndGetCookie(app, 'onb_me2', 'onb_me2@example.com');
    const token = generateToken(userId, 'onb_me2');
    ...
    expect(res.status).toBe(200);
    expect(res.body.user.onboardingCompletedAt).not.toBeNull();
    expect(Number.isNaN(new Date(res.body.user.onboardingCompletedAt).getTime())).toBe(false);
  });
});

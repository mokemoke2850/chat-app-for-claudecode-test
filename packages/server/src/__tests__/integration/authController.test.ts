...
describe('POST /api/auth/onboarding/complete', () => {
  it('正常: 認証済ユーザーが呼ぶと 200 を返し onboardingCompletedAt が設定される', () => {
    // TODO
  });

  it('正常: 2 回呼んでも 200 を返す（冪等性）', () => {
    // TODO
  });

  it('正常: 呼び出し後の me レスポンスで onboardingCompletedAt が ISO 文字列になる', () => {
    // TODO
  });

  it('異常: 非ログイン状態は 401 を返す', () => {
    // TODO
  });
});

describe('GET /api/auth/me（オンボーディング関連の追加項目）', () => {
  it('未完了ユーザーのレスポンスに onboardingCompletedAt: null が含まれる', () => {
    // TODO
  });

  it('完了済ユーザーのレスポンスに onboardingCompletedAt が ISO 文字列で含まれる', () => {
    // TODO
  });
});

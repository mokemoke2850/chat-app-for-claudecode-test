...
const BASE_SELECT = `
  SELECT
    ...
    t.created_by, t.position, t.created_at, t.updated_at
  FROM tasks t
...
export async function createTask(createdBy: number, input: CreateTaskInput): Promise<Task> {
  const { title, description, assigneeId, dueAt, sourceMessageId } = input;
  ...
  const result = await queryOne<{ id: number }>(
    `INSERT INTO tasks (title, description, assignee_id, due_at, source_message_id, created_by, position)

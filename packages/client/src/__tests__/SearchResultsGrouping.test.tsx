...
describe('検索結果グルーピング', () => {
  ...
  describe('エッジケース', () => {
    ...
    it.todo('検索結果が 0 件のときグルーピング切替 UI が表示されないかグレーアウト（仕様未定）');
    it.todo('グルーピング状態が localStorage に保存されて次回も維持される（優先度低）');
    it.todo('全メッセージが同一送信者の場合、送信者別でグループが1つになる');
    it.todo('全メッセージが同一日付の場合、日付別でグループが1つになる');
  });
});

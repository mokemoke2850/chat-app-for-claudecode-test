...
describe('SearchPage', () => {
  ...
  describe('Issue #249: 初期表示の空状態UI', () => {
    describe('初期マウント時 (クエリ空・フィルタ未設定)', () => {
      it.todo('「見つかりませんでした」が表示されない');
      it.todo('空状態用 UI (使い方ヒントまたは保存ビュー一覧) が表示される');
      it.todo('api.messages.search が呼ばれない');
    });

    describe('検索実行後の表示', () => {
      it.todo('クエリを入力して検索した結果が 0 件のときに「見つかりませんでした」が表示される');
      it.todo('クエリ入力後に結果がある場合は空状態 UI が消えて結果一覧が表示される');
    });

    describe('フィルタ設定時の表示切替', () => {
      it.todo(
        'フィルタ (送信者/添付/日付/タグなど) を設定した時点で空状態 UI から結果ペインに切り替わる',
      );
      it.todo('フィルタ設定後の結果が 0 件のときは「見つかりませんでした」が表示される');
    });

    describe('リセット動作', () => {
      it.todo('リセットボタン押下でクエリとフィルタがクリアされ空状態 UI に戻る');
      it.todo('リセット後は「見つかりませんでした」が表示されない');
    });
  });
});

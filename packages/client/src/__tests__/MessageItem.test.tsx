...
const dummyUsers: User[] = [
  { id: 1, username: 'alice', email: 'alice@example.com', avatarUrl: null, createdAt: '2024-01-01T00:00:00Z' },
  { id: 2, username: 'bob', email: 'bob@example.com', avatarUrl: null, createdAt: '2024-01-01T00:00:00Z' },
];
...
describe('MessageItem', () => {
  describe('削除済みメッセージ', () => {
    it('isDeleted=true のとき "This message was deleted." を表示する', () => {
      render(
        <MessageItem message={makeMessage({ isDeleted: true })} currentUserId={1} users={dummyUsers} />,
      );
  ...
  describe('通常メッセージの表示', () => {
    ...
    it('isEdited=true のとき "(edited)" を表示する', () => {
      render(
        <MessageItem message={makeMessage({ isEdited: true })} currentUserId={1} users={dummyUsers} />,
      );
    ...
    it('isEdited=false のとき "(edited)" を表示しない', () => {
      render(<MessageItem message={makeMessage({ isEdited: false })} currentUserId={1} users={dummyUsers} />);
  ...
  describe('自分のメッセージ（currentUserId === message.userId）', () => {
    ...
    it('Delete ボタンをクリックすると socket.emit("delete_message") が呼ばれる', async () => {
      render(
        <MessageItem message={makeMessage({ id: 42, userId: 1 })} currentUserId={1} users={dummyUsers} />,
      );
  ...
  describe('メッセージコンテンツのレンダリング', () => {
    ...
    it('content が不正な JSON のとき raw テキストとしてフォールバック表示する', () => {
      ...
      render(
        <MessageItem message={makeMessage({ content: rawContent })} currentUserId={1} users={dummyUsers} />,
      );

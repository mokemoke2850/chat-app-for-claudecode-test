...
describe('AdminCharts: 管理ダッシュボードのグラフ表示（Issue #271）', () => {
  ...
  describe('チャンネル別投稿ボリューム（次フェーズ）', () => {
    it.skip('チャンネル別の時系列を取得して同一グラフ上に複数系列で表示する', () => { /* see #338 */ });
    it.skip('チャンネル数が多い場合でも凡例（legend）が表示される', () => { /* see #338 */ });
    it.skip('対象チャンネルが 0 件の場合は空状態フォールバックを表示する', () => { /* see #338 */ });
  });

  describe('チャンネル別 Top N 横棒グラフ（次フェーズ）', () => {
    it.skip('上位 N チャンネルの投稿数が降順で横棒グラフに描画される', () => { /* see #338 */ });
    it.skip('チャンネル名と件数が読み取り可能な形でラベル表示される', () => { /* see #338 */ });
    it.skip('Top N の件数（N=10 など）が API 呼び出しの limit と一致する', () => { /* see #338 */ });
    it.skip('期間内に投稿が無い場合は空状態フォールバックを表示する', () => { /* see #338 */ });
  });
  ...
  describe('レイアウト・アクセシビリティ', () => {
    ...
    it.skip('期間フィルタは統計タブ全体（数値カード・グラフ）で共有される（次フェーズで強化）', () => { /* see #338 */ });
  });

  describe('Promise の安定化（React 19）', () => {
    it.skip('期間フィルタが変わらない限り時系列 API は再呼び出しされない（useMemo で安定化）', () => { /* see #338 */ });
    it.skip('タブ切替で再マウントされても同じ期間なら追加の API 呼び出しは発生しない', () => { /* see #338 */ });
  });
});

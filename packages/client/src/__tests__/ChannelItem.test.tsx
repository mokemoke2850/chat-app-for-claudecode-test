...
describe('ChannelItem', () => {
  ...
  describe('未読バッジ', () => {
    it('unreadCount > 0 かつ mentionCount === 0 のとき未読数バッジが表示される', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel({ unreadCount: 3, mentionCount: 0 })} />);
    ...
    it('unreadCount が 9 以下のとき実数が表示される', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel({ unreadCount: 5, mentionCount: 0 })} />);
      expect(screen.getByText('5')).toBeInTheDocument();
    });

    it('unreadCount が 10 以上のとき「9+」と表示される', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel({ unreadCount: 10, mentionCount: 0 })} />);
  ...
  describe('メンションバッジ', () => {
    it('mentionCount > 0 のときメンションバッジが表示される', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel({ unreadCount: 1, mentionCount: 2 })} />);
      expect(screen.getByText('2')).toBeInTheDocument();
    });

    it('mentionCount が 9 以下のとき実数が表示される', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel({ unreadCount: 5, mentionCount: 3 })} />);
      expect(screen.getByText('3')).toBeInTheDocument();
    });

    it('mentionCount が 10 以上のとき「9+」と表示される', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel({ unreadCount: 5, mentionCount: 10 })} />);
      expect(screen.getByText('9+')).toBeInTheDocument();
    });

    it('mentionCount > 0 のとき未読数バッジは表示されない', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel({ unreadCount: 5, mentionCount: 2 })} />);
  ...
  describe('ピン留めアクション', () => {
    it('行にホバーするとピン留めボタンが表示される', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel()} isHovered={true} isPinned={false} />);
    ...
    it('ピン留めボタンをクリックすると onPin が呼ばれる', async () => {
      const onPin = vi.fn();
      render(<ChannelItem {...defaultProps} channel={makeChannel({ id: 42 })} isHovered={true} isPinned={false} onPin={onPin} />);
    ...
    it('isPinned=true のときピン留め解除ボタンが表示される', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel()} isHovered={true} isPinned={true} />);
      expect(screen.getByRole('button', { name: 'ピン留めを解除' })).toBeInTheDocument();
    });

    it('ピン留め解除ボタンをクリックすると onUnpin が呼ばれる', async () => {
      const onUnpin = vi.fn();
      render(<ChannelItem {...defaultProps} channel={makeChannel({ id: 99 })} isHovered={true} isPinned={true} onUnpin={onUnpin} />);
  ...
  describe('プライベートチャンネルのメンバー管理', () => {
    it('isPrivate=true の行にホバーするとメンバー管理ボタンが表示される', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel({ isPrivate: true })} isHovered={true} />);
      expect(screen.getByRole('button', { name: 'メンバー管理' })).toBeInTheDocument();
    });

    it('isPrivate=false の行にはメンバー管理ボタンが表示されない', () => {
      render(<ChannelItem {...defaultProps} channel={makeChannel({ isPrivate: false })} isHovered={true} />);
    ...
    it('メンバー管理ボタンをクリックすると onOpenMembersDialog が呼ばれる', async () => {
      ...
      render(<ChannelItem {...defaultProps} channel={channel} isHovered={true} onOpenMembersDialog={onOpenMembersDialog} />);

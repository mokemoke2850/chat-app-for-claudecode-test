...
7. 実装前のテスト失敗確認（RED）
8. プログラム実装
9. 実装後のテスト成功確認（GREEN）・ビルド確認
10. 高リスク変更の独立 AI 実装レビュー
11. Playwright による画面動作確認
12. PR 作成
```
...
**7. 実装前のテスト失敗確認（RED）**
- 承認済みスケルトンをテストコードへ変換した直後、プログラム実装を変更する前に対象テストを実行する
- 新規・変更テストが、未実装の期待動作を理由として失敗することを確認し、実行コマンドと代表的な失敗内容を記録する
- テスト自体の構文エラー、モック不足、環境障害による失敗は RED とみなさず、テストを修正して再実行する
- 既存実装がすでに期待動作を満たしてテストが成功する場合は、重複実装せず、その証拠と追加実装が不要な理由を記録する
- 技術的に先に最小限のテスト用インターフェースが必要な場合も、振る舞いの本実装は行わず、可能な限り RED を観測してから進む
...
**9. 実装後のテスト成功確認（GREEN）・ビルド確認**
- RED を確認したものと同じ対象テストを再実行し、実装によって成功へ変わったことを確認する
